...
# The district-level dataframes were processed the same way as the dataframes at region-level.
...
# writes the processed dataframes into csv files
...
# Select useful column(s)
zim_district <- zim_district %>% 
  select(District) 

# join the dataframe and the shapefile by district
...
## Zim Region Map
This is the map of Zimbabwe at agro-ecological level
```{r}
...
# These are processed dataframes that filtered with specific dates 
GrSs2011 <- read_csv("./Data/EVI_region_GrSs2011.csv") # from Oct 2010 to May 2011
GrSs2017 <- read_csv("./Data/EVI_region_GrSs2017.csv") # from Oct 2016 to May 2017

# joining dataframe with shapefile
GrowSs<- GrSs2011 %>% 
  inner_join(GrSs2017, by = "Region")
...
GrSs2011Line <- EVI_region_long %>% 
  ...
  # create a new column that has the right order of time
...
# Rename variables                             
GrSs2011Line$Month[which(GrSs2011Line$Month=="10")] <- "Oct 2010"
GrSs2011Line$Month[which(GrSs2011Line$Month=="11")] <- "Nov 2010"
GrSs2011Line$Month[which(GrSs2011Line$Month=="12")] <- "Dec 2010"
GrSs2011Line$Month[which(GrSs2011Line$Month=="01")] <- "Jan 2011"
GrSs2011Line$Month[which(GrSs2011Line$Month=="02")] <- "Feb 2011"
GrSs2011Line$Month[which(GrSs2011Line$Month=="03")] <- "Mar 2011"
GrSs2011Line$Month[which(GrSs2011Line$Month=="04")] <- "Apr 2011"
GrSs2011Line$Month[which(GrSs2011Line$Month=="05")] <- "May 2011"
...
# write.csv(GrSs2011Line,"./Data/GrSs2011Line.csv", row.names = FALSE)
...
GrSs2011Line %>% 
  ...
  labs(title = "Maximum EVI During Growing Season 2010-2011", color =  "Region") +
  xlab("Time(Month)") +
  ylab("Maximum EVI") 
...
```{r}
# Same thing, just for 2016-2017 growing season
...
GrSs2017Line$Month[which(GrSs2017Line$Month=="10")] <- "Oct 2016"
GrSs2017Line$Month[which(GrSs2017Line$Month=="11")] <- "Nov 2016"
GrSs2017Line$Month[which(GrSs2017Line$Month=="12")] <- "Dec 2016"
GrSs2017Line$Month[which(GrSs2017Line$Month=="01")] <- "Jan 2017"
GrSs2017Line$Month[which(GrSs2017Line$Month=="02")] <- "Feb 2017"
GrSs2017Line$Month[which(GrSs2017Line$Month=="03")] <- "Mar 2017"
GrSs2017Line$Month[which(GrSs2017Line$Month=="04")] <- "Apr 2017"
GrSs2017Line$Month[which(GrSs2017Line$Month=="05")] <- "May 2017"
...
GrSs2017Line %>% 
  ...
  geom_dl(aes(label = Region), method = list(dl.combine("first.points", "last.points")), cex = 0.8) +
  scale_color_viridis_d(option = "H") +
  labs(title = "Maximum EVI During Growing Season 2016-2017", color =  "Region") +
  xlab("Time(Month)") +
  ylab("Maximum EVI") 
...
# write.csv(EVIGrow2011,"./Data/EVIGrow2011.csv", row.names = FALSE)

RgMaxEVI2011 <- EVIGrow2011 %>% 
  ggplot() +
  geom_sf(size = 0.15, color = "black", aes(fill = MaxEVI)) + # adding the geometric layer
  coord_sf() +
  scale_fill_viridis_c(option = "D", direction = -1, limits = c(0.36, 0.52), breaks=c(0.36, 0.38, 0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52)) + # manually adding the same scale for the two maps
  ggtitle("Maximum EVI in Zimbabwe-Growing Season 2010-2011")
...
RgMaxEVI2017 <- EVIGrow2017 %>% 
  ...
  coord_sf() +
  scale_fill_viridis_c(option = "D", direction = -1, limits = c(0.36, 0.52), breaks=c(0.36, 0.38, 0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52)) +
  ggtitle("Maximum EVI in Zimbabwe-Growing Season 2016-2017")
...
## Zim Region Map
Filled the shapefile with color by the agro-ecological regions
```{r}
RegionMap <- zim_region %>% 
  ggplot() +
  geom_sf(size = 0.15, color = "black", aes(fill = Region)) +
...
# ---------------------------------------------------
These are the vis that weren't used for the Shiny App

## 1. Monthly map for Year 2011 and 2017
```{r, echo=FALSE}
# Maximum evi by month
MaxMonthlyEVI2011 <- MonthlyEVIbyDis %>% 
  filter(Year == 2011) %>% # filter the dataframe for wanted year(s) 
  arrange(Month) %>% 
  ggplot() +
  geom_sf(size = 0.1, color = "black", aes(fill = MaxEVI)) + # this adds the geometric layer from the shapefile
  coord_sf() +
...
Leaflet map for GrowingSeason2011
