...
PRECIP_district <-  full_join (zim_district, PRECIP_monthly_2011, by = "District")
```


Mapping NDVI [NOT NEEDED for Josue]
```{r}
...
Josue's Adaption 
```{r}
# Maximum Precipitation by month
AvgMonthlyPRECIP <- PRECIP_district %>% 
  filter(Type == "AveragePRECIP") %>% 
  arrange(Month) %>% 
  ggplot() +
  ...
  scale_fill_viridis_c(option = "G", direction = -1) +
  facet_wrap(~Month) +
  ggtitle("Average Precipitation in Zimbabwe - 2011")
AvgMonthlyPRECIP


jpeg(filename = "AvgMonthlyPRECIP.jpeg", 
    res = 500,
     bg = "white")
AvgMonthlyPRECIP
dev.off()
