...
mydat_longg <- mydat_long %>% 
  mutate(
         ...
         ExtremelyDry = case_when(Moisture <=40 ~ 1,
                                  TRUE ~ 0),
         )
...
# Grouped
 ggplot(df2, aes(fill=time, y=value, x=region)) + 
     ...
     ggtitle("% Soil Moisture Conditions of the 2016-17 Growing Season after Planting Time") +
...
# attributes(mydat_long2$newDate)
 ...
 #         lat = map_dbl(geometry, ~st_point_on_surface(.x)[[2]]))
  ...
  ggplot(data = total) +
   ...
   coord_sf() +
    scale_fill_viridis(option = "viridis", direction = -1, limits=c(0,100), breaks=c(20,40,60,80,100), labels=c("20","40", "60", "80", "100")) +
     ggtitle("Average % Soil Moisture of The 2016-17 Growing Season after planting time") + labs(caption = "3 day: NASA-USDA Enhanced SMAP Global") +
     guides(fill=guide_legend(title="Average Percent Soil Moisture (%)"))+theme_bw()
  
  
     
```
Write files for shiny
```{r}
