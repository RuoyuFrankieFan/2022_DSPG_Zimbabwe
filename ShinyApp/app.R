       ...
       provide a more obvious descriptor for the poorest people in a given area."),
               ...
               ")   
                              ...
                              tabPanel(strong("Soil Moisture"),
                                     ...
                                     tabPanel("Surface Soil Moisture",
                                             ...
                                             # ),
                                              ...
                                              fluidRow(
                                                style = "margin-left: 100px; margin-right: 100px;",
                                                h1(strong("Average Surface Soil Moisture During Planting for the 2016-17 Growing Season"), align="center",
                                                   style = "font-size: 25px;"),
                                                align = "left"
                                              ),   
                                     ...
                                     fluidRow(
                                       style = "margin-left: 100px; margin-right: 100px;",
                                       h1(strong("Surface Soil Moisture Period Conditions"), align="center",
                                          style = "font-size: 25px;"),
                                       align = "left"
                                     ), 
                                     ...
                                     fluidRow(
                                       style = "margin-left: 100px; margin-right: 100px;",
                                       h1(strong("Surface Soil Moisture Across Time"), align="center",
                                          style = "font-size: 25px;"),
                                       align = "left"
                                     ), 
                  ...
                  tabPanel("Percent Soil Moisture",
                           ...
                           fluidRow(
                             style = "margin-left: 100px; margin-right: 100px;",
                             h1(strong("Average Percent Soil Moisture"),align="center",
                                style = "font-size: 25px;"),
                             align = "left"
                           ), 
                           ...
                           fluidRow(
                             style = "margin-left: 100px; margin-right: 100px;",
                             h1(strong("Percent Soil Moisture period conditions"),align="center",
                                style = "font-size: 25px;"),
                             align = "left"
                           ), 
                           ...
                           fluidRow(
                             style = "margin-left: 100px; margin-right: 100px;",
                             h1(strong("Percent Soil Moisture Across Time"),align="center",
                                style = "font-size: 25px;"),
                             align = "left"
                           ), 
                                   ...
                                   fluidRow(h1(strong("Components of the MPI"), align = "center"),
                                            ...
                                            p("This graphic shows a map of the relevant components of MPI at the", strong("district-level"),". Our study uses ", strong("district-level")," measures of various MPI components to explore their association with the three remotely sensed indices of concern. We limit only to those components that assign an equal weight to urban and rural households. Otherwise, components with unequal weights may over-/underestimate the severity of deprivation if a district contains predominantly urban (rural) households. For example, component Lack of Land is assigned a weight of zero to urban households, so districts (such as Bulawayo) that are mostly urban will appear to be less deprived in this component than more rural districts. The components we examine in this study are:"),
                                              tags$ul(
                                                ...
                                                tags$li(strong("Unenrolled Child (Education Dropout):")," an indicator variable for whether the household has a child aged 7-11 who is not enrolled in school."),
...
For more details on the gap (\\(M_{1}\\)), and severity of poverty (\\(M_{2}\\)), please refer to ", a(href="https://dspgtools.shinyapps.io/dspg21zimbabwe/","Using PICES Data to Visualize District Level Multidimensional Poverty in Zimbabwe",target='_blank'), ".")
              ...
              tabPanel(strong("Regressions"),
                       ...
                       fluidRow(
                         h1(strong("District-Level MPI & Precipitation"), 
                            style = "font-size:35px;"), align="center",
                         style = "margin-left: 0px; margin-right: 0px;",
                         column(12
                       ...
                       fluidRow(
                         ...
                         column(
                           ...
                           align="justify",
                           withMathJax(),
                           p("In this section, we present the results from regression analysis of total precipitation (measured in 100 mm) on MPI and its selected components, using", strong("district-level"), "data."),
                           p("Since the majority of the crop-fields in Zimbabwe rely on rain-fed irrigation, rainfall can be regarded as an input in the production function of maize. Therefore, the quantity of rainfall will directly affect crop yield, and by extension, crop yield is likely to affect the socio-economic conditions of agricultural households in Zimbabwe. Therefore, we expect there to be a meaningful association between rainfall during the maize growing season and poverty status in the following year of surveyed households. For instance, inadequate rainfall in the prior maize growing season could mean that households now own fewer assets (Lack of Household Assets component) if part of their stock of assets were sold off to make up for lost income from poor yield."),
                           p("We estimate the following regression model using Ordinary Least Squares (OLS) Estimation method:"),
                       ...
                       tabPanel(strong("Sensitivity Analysis 1"),
                                ...
                                fluidRow(
                                  ...
                                  column(3,
                                         ...
                                         p("Since literacy rate in Zimbabwe is quite high at 88.69 percent, and primary education completion rate is 89 percent, education-adjusted MPI measures were generated by replacing the “No Primary Education” component with a “No High School Education” component defined as no one in the household having completed high school."), 
                                         p("The regression coefficients using the adjusted MPI measures are presented in Figures 1A, 2A and 3A. When compared to Figures 1, 2 and 3, respectively, we observe that the overall trend is consistent. However, in some cases the associations between cumulative monthly rainfall and the education-adjusted MPI measures are slightly less significant.")
                                  )
                                )
...
output$compo_MPI_11 <- renderLeaflet({
  leaflet(joined_zim) %>% addTiles() %>%  
    ...
    addPolygons(fillColor = ~mypal(joined_zim$g0_edu_dropout_k3), weight = 1, smoothFactor = 0.5, label = paste("", joined_zim$District_name, ":", round(joined_zim$g0_edu_dropout_k3, digits = 3)),
                opacity = 1.0, fillOpacity = 0.5,
                highlightOptions = highlightOptions(color = "black", weight = 2,
                                                    bringToFront = TRUE), group="Unerolled Child")  %>%  
    ...
    addLayersControl(baseGroups = c("No Primary Education","Unenrolled Child", "Chronic Ilness","Health Visit", "Household Assets","Service Access"), 
                     options = layersControlOptions(collapsed = FALSE), position = "topright") %>%
    #hideGroup("M0")%>% 
    hideGroup("Unenrolled Child")%>% 
...
output$compo_MPI_11_m1 <- renderLeaflet({
  leaflet(joined_zim) %>% addTiles() %>%  
    ...
    addPolygons(fillColor = ~mypal(joined_zim$g1_edu_dropout_k3), weight = 1, smoothFactor = 0.5, label = paste("", joined_zim$District_name, ":", round(joined_zim$g1_edu_dropout_k3, digits = 3)),
                opacity = 1.0, fillOpacity = 0.5,
                highlightOptions = highlightOptions(color = "black", weight = 2,
                                                    bringToFront = TRUE), group="Unenrolled Child")  %>%  
