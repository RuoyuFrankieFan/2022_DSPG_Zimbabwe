...
## Read in the files
```{r}
soil_source <- read_csv("./Data/soilsub_ts_Zimb.csv")
...
soil_wide <- soil_source %>% 
  select(NAME_2, `NASA_USDA_SMAP_SM20150502_20150504_susm`:`NASA_USDA_SMAP_SM20211229_20211231_susm`) %>% #select needed columns--Districts and observations
...
soil_long <- gather(soil_wide, Date, SoilMoisture, `NASA_USDA_SMAP_SM20150502_20150504_susm`:`NASA_USDA_SMAP_SM20211229_20211231_susm`)%>% 
  separate( col=Date, into=c('NASA', 'USDA', "SMAP", "DateBegin", "DateEnd", "susm"), sep='_') %>% # Parsed it into Year/Month/Day
  select(-c(NASA, USDA, SMAP, susm)) #drop the string segment
...
3.0 write csv
```{r}
write.csv(soil_annual,"./Data/soil_annual.csv", row.names = FALSE)
```
...
4.0 write csv
```{r}
write.csv(soil_monthly,"./Data/sub_soil_monthly.csv", row.names = FALSE)
```


5. Subset of Monthly Data- 2016-2018
```{r}
`soil_monthly_2016_2018` <- soil_monthly %>% 
  filter(YearBegin == 2016|YearBegin == 2017|YearBegin == 2018) %>% 
  arrange(YearBegin, MonthBegin)

# write.csv(soil_monthly,"./Data/sub_soil_monthly_2016-2018.csv", row.names = FALSE)
```

5.0 write csv
```{r}
write.csv(soil_monthly_2016_2018,"./Data/sub_soil_monthly_2016_2018.csv", row.names = FALSE)
```

5. Subset of Monthly Data- 2017
```{r}
`soil_monthly_2017` <- soil_monthly %>% 
