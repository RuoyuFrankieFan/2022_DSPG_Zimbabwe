...
## Read in the files
```{r}
soil_source <- read_csv("./Data/soilm_perc_ts_Zimb.csv")
...
soil_wide <- soil_source %>% 
  select(NAME_2, `NASA_USDA_SMAP_SM20150502_20150504_smp`:`NASA_USDA_SMAP_SM20211229_20211231_smp`) %>% #select needed columns--Districts and observations
...
soil_long <- gather(soil_wide, Date, SoilMoisture, `NASA_USDA_SMAP_SM20150502_20150504_smp`:`NASA_USDA_SMAP_SM20211229_20211231_smp`)%>% 
  separate( col=Date, into=c('NASA', 'USDA', "SMAP", "DateBegin", "DateEnd", "smp"), sep='_') %>% # Parsed it into Year/Month/Day
  select(-c(NASA, USDA, SMAP, smp)) #drop the string segment
...
4.0 write csv
```{r}
write.csv(soil_monthly,"./Data/perc_soil_monthly.csv", row.names = FALSE)
```
...
# write.csv(soil_monthly,"./Data/perc_soil_monthly_2016-2018.csv", row.names = FALSE)
```

5.0 write csv
```{r}
write.csv(soil_monthly_2016_2018,"./Data/perc_soil_monthly_2016_2018.csv", row.names = FALSE)
```
